...
def plot_fig_par(num_iters = 25,d_list = [10,1000,10000],folder_path = 'result/',alphas=[],latex_text = r"$\alpha$"):
    num_d  = len(d_list)
    data_dict = {}
    # methods = ['Method 1', 'Method 2','RRM Logistic Regression','RGD Logistic Regression',\
    #            'Two-Stage Approach','PerGD','RRM with Neural Networks']
    for alpha in alphas:
    ...
    for c in range(num_d):
        ...
        for i,(file_key, inner_dict) in enumerate(data_dict.items()):
            for j in range(1,num_iters):
                ...
                plot_step(j, offset, acc_list_start_avg, acc_list_end_avg, f'{latex_text} = {file_key}',colors[i],markers[i],num_iters,c)
    ...
    for c in range(num_d):
        ...
        for i, alpha in enumerate(alphas):
            ...
            plot_acc(acc_list_start_avg[c],acc_list_start_std[c],colors[i],markers[i],linestyles[i],f'{latex_text} = {alpha}')
    ...
    for c in range(num_d):
        ...
        for i, alpha in enumerate(alphas):
            ...
            acc_list_start_avg = inner_dict.get('acc_list_start_avg')
            acc_list_start_std = inner_dict.get('acc_list_start_std')
            plot_acc(acc_list_start_avg[c],acc_list_start_std[c],colors[i],markers[i],linestyles[i],f'{latex_text} = {alpha}',std = 2)
            max_element = max(max_element,np.max(np.maximum.reduce(acc_list_start_avg[c])))
            min_element = min(min_element,np.min(np.minimum.reduce(acc_list_start_avg[c])))
        plt.xlabel('Iteration', fontsize = 18)
        plt.ylabel('Accuracy', fontsize = 18)
    ...
    for c in range(num_d):
        ...
        for i, alpha in enumerate(alphas):
            ...
            plot_model_gap(model_gaps_avg[c],model_gaps_std[c],colors[i],markers[i],linestyles[i],f'{latex_text} = {alpha}')
        plt.xlabel('Iteration', fontsize = 18)
    ...
    for c in range(num_d):
        ...
        for i, alpha in enumerate(alphas):
            ...
            model_gaps_avg = inner_dict.get('model_gaps_avg')
            model_gaps_std = inner_dict.get('model_gaps_std')
            plot_model_gap(model_gaps_avg[c],model_gaps_std[c],colors[i],markers[i],linestyles[i],f'{latex_text} = {alpha}',std = 2)
        plt.xlabel('Iteration', fontsize = 18)
        plt.ylabel('Model Consistency', fontsize = 18)
        plt.tick_params(labelsize=18)
        plt.ylim(0, 1)
